...
#*----- Description :   RAM probe for Watcher-Soft                                      ****#
#*******************************************************************************************#


'''
DESCRIPTION :
  * Watcher-Soft plugin used to return machine "MEM (Total, User, System)".
'''

#DEFAULT_OPTIONS=" -w 12747000 -c 16000000"
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="""
        Check and return MEM usage per connected user.
        """,
        usage="""
            MEM_probe.py -w 8000000000 -c 12000000000
        """,
        epilog="version {}, license {}, copyright {}, credits {}".format(__version__,__license__,__copyright__,__credits__))
    parser.add_argument('-w', '--warning', type=int, nargs='?', help='warning trigger', default=8000000000)
    parser.add_argument('-c', '--critical', type=int, nargs='?', help='critical trigger', default=12000000000)
    ...
    for CurrentUser in ListCurrentUsers:
        ...
        if memory_consom[0] >= args.critical:
            Global_State = 2
        if Global_State == 0:
            if memory_consom[0] >= args.warning:
                Global_State = 1

        if PosList == 0:
            Out += "L'utilisateur " + CurrentUser + " consomme " + str(memory_consom[0]) + " de RAM "
            Perf += "| " + CurrentUser + "_mem=" + str(memory_consom[0]) + ";" + str(args.warning) + ";" + str(args.critical) + "; " 
        else:
            Out += ", L'utilisateur " + CurrentUser + " consomme " + str(memory_consom[0]) + " de RAM"
            Perf += CurrentUser + "_mem=" + str(memory_consom[0]) + ";" + str(args.warning) + ";" + str(args.critical) + "; " 
